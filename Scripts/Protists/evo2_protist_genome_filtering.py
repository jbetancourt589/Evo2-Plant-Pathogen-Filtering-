#!/usr/bin/env python3
"""
Filter genomes for Evo2-trained protist assemblies.

Logic:
1. Read trained protist assembly rows, with --limit available for smaller test runs.
...
DEFAULT_TRAINED_PROTISTS = PROJECT_ROOT / "Results" / "Protists" / "evo2_protists_names_and_assemblies.csv"
...
DEFAULT_OUTPUT = PROJECT_ROOT / "Results" / "Protists" / "evo2_protist_genome_filtered.fasta"
DEFAULT_SUMMARY_OUTPUT = PROJECT_ROOT / "Results" / "Protists" / "evo2_protist_genome_filtered_summary.csv"
DEFAULT_LIMIT = 0
...
def read_target_assemblies(path: Path, limit: int) -> list[TargetAssembly]:
    """Read Evo2-trained protist rows, with limit=0 meaning no limit."""
    targets = []
...
def load_ncbi_metadata(path: Path, targets: list[TargetAssembly]) -> dict[str, NcbiAssembly]:
    ...
    for row in read_assembly_summary_rows(path):
        ...
        metadata = NcbiAssembly(assembly_id=assembly_id, paired_assembly_id=paired_assembly_id, ftp_path=ftp_path)
...
def main() -> None:
    ...
    with args.output.open("w", encoding="utf-8", newline="\n") as output_file:
        for index, target in enumerate(targets, start=1):
            ...
            print(f"[{index}/{len(targets)}] Downloading/filtering {target.organism_name} ({target.assembly_id})")
            ...
            stats_rows.append(filter_cached_fasta(cached_path, output_file, target, source_url, annotation_result))
